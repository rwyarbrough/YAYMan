...
public class ImageListItem extends ImageIcon {
    ...
    public ImageListItem(String orig, String disp, final String thum, JList list) throws MalformedURLException {
        super();
        thumbList = list;
    ...
    public ImageListItem(Artwork orig, Artwork disp, Artwork thum, JList list) throws MalformedURLException {
        this(orig.getFilePath(), disp.getFilePath(), thum.getFilePath(), list);
    }

    public ImageListItem(Artwork orig, Artwork disp, JList list) throws MalformedURLException {
        this(orig, disp, disp, list);
    }

    public ImageListItem(final Artwork art, JList list) throws MalformedURLException {
        this(art, art, list);
    }

    public ImageListItem(Banner banner, JList list) throws MalformedURLException {
        this(banner.getUrl(), banner.getUrl(), banner.getUrl().replace("/banners/", "/banners/_cache/"), list);
    }

    public ImageListItem(File file, JList list) throws MalformedURLException {
        super();
        mode = ImageListItem.LOCALMODE;
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(yayman.YAYManApp.class).getContext().getResourceMap(ImageListItem.class);
        waitImage = resourceMap.getImageIcon("thumbnailWaitIcon").getImage();
    ...
    private void setImage(final String path) throws MalformedURLException {
    ...
    private Image getScaledImage(String path) throws MalformedURLException {
       // try {
            Image img;
